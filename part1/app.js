...
app.get('/api/walkers/summary', async (req, res) => {
    try {
        const [walkers_summary] = await db.query(`SELECT u.username AS walker_username, COUNT(req.request_id) FILTER (WHERE req.status = 'completed') AS completed_walks, COUNT(r.rating_id) AS total_ratings, AVG(r.rating) AS average_rating FROM Users u LEFT JOIN WalkRatings r On u.user_id = `);
